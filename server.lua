...
function CreateBlipForGroup(groupID, name, label, coords, sprite, color, scale, route)
    if groupID == nil then return print("CreateBlipForGroup was sent an invalid groupID :"..groupID) end
    local members = getGroupMembers(groupID)
    for i=1,#members do
        TriggerClientEvent("groups:createBlip", members[i], name, label, coords, sprite, color, scale, route)
    end
end
